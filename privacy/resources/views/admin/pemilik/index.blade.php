                        ...
                        <div class="row">
                            <div class="col-md-8">
                                <div class="form-group">
                                    {{ Form::label('kode_pemilik', 'Vendor:') }}
                                    {{Form::select('kode_pemilik', $Vendor->sort(), null, ['class'=> 'form-control select2','style'=>'width: 100%','placeholder' => '','id'=>'Vendor1','required'])}}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-8">
                            <div class="form-group">
                                {{ Form::label('kode_pemilik', 'Vendor:') }}
                                {{Form::select('kode_pemilik', $Vendor->sort(), null, ['class'=> 'form-control select2','style'=>'width: 100%','placeholder' => '','id'=>'Vendor2','required'])}}
                            </div>
                        </div>
                    </div>
                </div>
        ...
        $(document).ready(function(){
            ...
            $('#data-table tbody').on( 'click', 'tr', function () {
                if ( $(this).hasClass('selected bg-gray') ) {
                    ...
                    var colom = select.find('td:eq(1)').text();
                    var data = $('#data-table').DataTable().row(select).data();
                    var kode_pemilik = data['kode_pemilik'];
                    var item = colom;
                    var add = $("#addpemilik").attr("href","http://localhost/gui_front_pbm_laravel/admin/pemilik/"+kode_pemilik+"/detail");
                    if(item > 0){
                        $('.add-button').show();
                        $('.hapus-button').hide();
                        $('.edit-button').show();
            ...
            $('#editpemilik').click( function () {
                ...
                var row = table.row( select );
                $.ajax({
                    url: '{!! route('pemilik.edit_pemilik') !!}',
                    type: 'POST',
                    data : {
                        'kode_pemilik': kode_pemilik
                    },
                    success: function(results) {
                        console.log(results);
                        $('#Vendor2').val(results.kode_pemilik).trigger('change');
                        $('#editform').modal('show');
                        }
            ...
            $('#hapuspemilik').click( function () {
                ...
                swal({
                    ...
                    if (e.value === true) {
                        $.ajax({
                            url: '{!! route('pemilik.hapus_pemilik') !!}',
                            type: 'POST',
                            data : {
                                'kode_pemilik': kode_pemilik
                            },
