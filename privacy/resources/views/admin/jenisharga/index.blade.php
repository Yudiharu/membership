            ...
            <div class="box ">
                <div class="box-body">
                    ...
                    <button type="button" class="btn btn-success btn-xs" data-toggle="modal" data-target="#addform">
                        <i class="fa fa-plus"></i> Jenis Harga</button>
                    @endpermission

                    <span class="pull-right">  
                        <font style="font-size: 16px;"><b>JENIS HARGA</b></font>
                    </span>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            {{ Form::label('Nama', 'Description:') }}
                            {{ Form::text('description', null, ['class'=> 'form-control','id'=>'Jenis1','required'=>'required', 'placeholder'=>'Kode Harga','autocomplete'=>'off', 'onkeypress'=>"return pulsar(event,this)"]) }}
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="modal-body">
                    ...
                    {{ Form::hidden('id', null, ['class'=> 'form-control','id'=>'Kode','readonly']) }}
                        <div class="col-md-8">
                            <div class="form-group">
                                {{ Form::label('Nama', 'Description:') }}
                                {{ Form::text('description', null, ['class'=> 'form-control','id'=>'Jenis2','required'=>'required', 'placeholder'=>'Kode Jenis Harga','autocomplete'=>'off', 'onkeypress'=>"return pulsar(event,this)"]) }}
                            </div>
                        </div>
                    </div>
                </div>
...
@push('js')
    ...
    $(function() {   
        ...
        $(document).ready(function(){
            ...
            $('#data-table tbody').on( 'click', 'tr', function () {
                if ( $(this).hasClass('selected bg-black') ) {
                    $(this).removeClass('selected bg-black');
                    $('.hapus-button').hide();
                    $('.edit-button').hide();
                }else {
                    table.$('tr.selected').removeClass('selected bg-black');
            ...
            $('#editjenis').click( function () {
                ...
                $.ajax({
                    url: '{!! route('jenisharga.edit_jenis') !!}',
                    type: 'POST',
                    data : {
                        'id': id
                    },
            ...
            $('#hapusjenis').click( function () {
                ...
                swal({
                    ...
                    if (e.value === true) {
                        $.ajax({
                            url: '{!! route('jenisharga.hapus_jenis') !!}',
                            type: 'POST',
                            data : {
                                'id': kode_sopir
                            },
        ...
        $('#EDIT').submit(function (e) {
            ...
            var formData = registerForm.serialize();
            $.ajax({
                url:'{!! route('jenisharga.ajaxupdate') !!}',
                type:'POST',
                data:formData,
                success:function(data) {
                    console.log(data);
