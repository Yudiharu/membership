...
class UsersDataTable extends DataTable
{
    ...
    public function query(User $model)
    {
        ...
        if($level != 'superadministrator'){
            return $model->newQuery()->with('roles','company')->select('id', 'name', 'username', 'email', 'kode_company', 'kode_lokasi' ,'level')->where('username',$username);
        }
        else{
            return $model->newQuery()->with('roles','company')->select('id', 'name', 'username', 'email', 'kode_company', 'kode_lokasi' ,'level');
        }
    }
